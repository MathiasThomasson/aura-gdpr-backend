...
def test_toms_crud_and_isolation():
    tenant_id, user_id, _ = create_tenant_and_user()
    override_user_dependency(app, get_current_user, tenant_id, user_id)

    created = client.post("/api/toms", json={"title": "TOM A"})
    assert created.status_code == 201
    item_id = created.json()["id"]

    listing = client.get("/api/toms")
    assert listing.status_code == 200
    assert any(i["id"] == item_id for i in listing.json())

    detail = client.get(f"/api/toms/{item_id}")
    assert detail.status_code == 200

    updated = client.patch(f"/api/toms/{item_id}", json={"description": "Updated"})
    assert updated.status_code == 200

    client.delete(f"/api/toms/{item_id}")
    override_user_dependency(app, get_current_user, *create_tenant_and_user()[:2])
    assert client.get(f"/api/toms/{item_id}").status_code == 404
