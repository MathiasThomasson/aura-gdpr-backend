...
def test_projects_crud_and_isolation():
    ...
    detail = client.get(f"/api/projects/{item_id}")
    assert detail.status_code == 200

    updated = client.patch(f"/api/projects/{item_id}", json={"status": "active"})
    assert updated.status_code == 200

    client.delete(f"/api/projects/{item_id}")
    override_user_dependency(app, get_current_user, *create_tenant_and_user()[:2])
    assert client.get(f"/api/projects/{item_id}").status_code == 404
